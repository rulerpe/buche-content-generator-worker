...
async function generateContentWithContext(
	...
	try {
		...
		const enhancedPrompt = `你是一个专业的情色文学创作助手。重点参考以下参考片段的写作风格、情节发展和表达方式来创作后续内容：

**参考片段（重点学习对象）：**
${snippetsText || '(无相关片段)'}

**从参考片段中学习：**
1. **写作风格和语言特色** - 模仿参考片段的叙述方式、句式结构和用词习惯
2. **情节推进技巧** - 学习参考片段如何发展情节、营造氛围和推动故事
3. **人物互动模式** - 参考片段中角色之间的对话方式、行为模式和情感表达
